...
@Getter
@Setter
public class VanillaBpProperties extends AdapterProperties {
  ...
  public void validatePropertiesFor(
    ...
    final var missingAdapters = defaultAdapters.stream()
  ...
  public void validateProperties(
    ...
    getWorkflowModules()
        ...
        .forEach(
            workflowModule -> {
              workflowModule.getDefaultAdapter().stream()
                  .filter(adapterId -> !adapterIds.contains(adapterId))
                  .forEach(
                      adapterId -> notConfiguredAdapters
                          .get(
              ...
              workflowModule
                  ...
                  .forEach(
                      workflow -> {
                        workflow.getDefaultAdapter().stream()
                            .filter(adapterId -> !adapterIds.contains(adapterId))
                            .forEach(
                                adapterId -> notConfiguredAdapters
                                    .get(
    ...
    knownWorkflowModuleIds.forEach(
        workflowModuleId -> {
          ...
          final var resourcesLocationOfModule = (defaultAdapterOfModule.isEmpty() ? getDefaultAdapter()
          ...
          getBpmnProcessIdsForWorkflowModule(workflowModuleId)
              .forEach(
                  bpmnProcessId -> {
                    final var defaultAdapter = getDefaultAdapterFor(
                        workflowModuleId,
                        bpmnProcessId
                    );
                    final var resourcesLocation = defaultAdapter.stream()
                        .filter(
