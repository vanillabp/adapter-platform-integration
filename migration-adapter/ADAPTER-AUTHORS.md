...
| `multiInstanceElementNames(module, process, taskDefinition)`                    | which iterations the methods serving that element want the ITEM of. Read your multi-instance elements and refuse a model which hands over none where a handler asks for one; an id your model does not know belongs to a caller and is no finding |
...
| `getBusinessKey()`          | the business key your BPMS keeps for this workflow, where it keeps one at all. A business key counts only where it carries the workflow aggregate's id, so the core compares the two and refuses the delivery where they disagree. Leave it out where your BPMS has no such thing, and answer it with the value you took the aggregate's id from where your BPMS keeps the id IN the business key (decision 69)                                                                   |
| `getProcessVersion()`       | matched against the `version` attribute of the annotations                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `getTaskParameter(name)`    | values the MODEL produced, never values of the aggregate. Return the value the way your BPMS deserialized it, and convert nothing. The core converts it into the type the handler declared, so one handler does not answer differently per BPMS (see "What a `@TaskParam` may be declared as" in [README.md](./README.md))                                                                                                                                                        |
...
Where your BPMS keeps a business key, write the aggregate's id into it whenever you start a
workflow, and report what it holds through `getBusinessKey()` whenever you hand a workflow back.
The rule the core enforces on that value is one sentence: a business key counts only where it
carries the workflow aggregate's id. A workflow started past VanillaBP can carry a key somebody
else chose, and then two values say different things about one instance. The core refuses such a
delivery rather than picking one of them, so nothing of yours has to notice it, and your BPMS then
does with the refusal what it does with any failing task. An empty answer contradicts nothing and
is the whole answer for a BPMS without business keys (decision 69).
...
anyway: the core would have no method to route it to, and it writes no delivery record for a task
nobody served, which is why `TaskDeliveryLog#openTasksOfAggregate` answers the open work of the
APPLICATION rather than the open work of the workflow (decision 54).
...
searchable yet (`PhaseTwoRetryLater`, with the window your BPMS may need) does not park the
