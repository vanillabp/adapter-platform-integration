 ...
 * <p>
 * Every method is a <code>default</code> method, which keeps hand-written
 * implementations source-compatible when methods are added to this interface. Most of
 * them throw an {@link UnsupportedOperationException} with a guiding message, and the
 * platform-provided implementations (e.g. based on Spring Data) override those; a
 * custom implementation overrides every method VanillaBP actually uses for its
 * aggregates. The ones which answer instead of throwing say so where they are
 * declared, and each of them answers what an application which does not implement it
 * would want.
 *
...
public interface AggregatePersistenceAware<A> {
  ...
  /**
   * Which state of this aggregate is the current one, right now, named the way the
   * application's own auditing names it: a Hibernate Envers revision, or a version an
   * application keeps itself.
   * <p>
   * VanillaBP asks while the caller's transaction is still open, so that a phase-two
   * call planned in it can remember which state its planner saw
   * ({@link PhaseTwoCall#askingForTheStateOfTheEvent(String)}). The dispatch of such a
   * call happens later, possibly much later, and
   * {@link #loadByIdAndAuditingId(Object, String)} is then asked for exactly that
   * state.
   * <p>
   * <b>This default does NOT throw</b> (unlike most of the others): it answers
   * <code>null</code>, which means "this persistence keeps no history". Nothing then
   * asks for an old state anywhere, and every load is the load it was before.
   * <p>
   * The answer is a {@link String} because it travels in the outbox entry, where
   * everything is a String. An application whose auditing numbers its revisions writes
   * the number down here and reads it back in the load.
   * <p>
   * When the id becomes valid is the application's business, not VanillaBP's: an
   * auditing which assigns its revision at flush time is asked before the flush, so the
   * application answers the revision it is going to write rather than one it has
   * already written.
   *
   * @param aggregate The aggregate whose current state is to be named
   * @return The auditing id of the aggregate's current state, or <code>null</code> where
   *         this persistence keeps no history
   */
  ...
  /**
   * Loads the aggregate as it was when the given auditing id was the current one - the
   * state somebody saw at the moment they planned a phase-two call
   * ({@link #getAuditingId(Object)}).
   * <p>
   * <b>This default does NOT throw</b> (unlike most of the others): it ignores the
   * auditing id and loads the aggregate as it is now, which is what every application
   * without an auditing wants and what VanillaBP did before this method existed.
   * <p>
   * Answer <code>null</code> where that state is not available any more, typically
   * because the auditing was cleaned up while the outbox entry was waiting. VanillaBP
   * then loads the current state and says so in a warning: a report carrying newer
   * values is better than no report at all.
   *
   * @param aggregateId The aggregate's ID (as returned by {@link #getAggregateId(Object)})
   * @param auditingId The auditing id of the wanted state
   * @return The aggregate as it was, or <code>null</code> if that state is gone
   */
  default A loadByIdAndAuditingId(
      final Object aggregateId,
      final String auditingId) {

    return loadById(aggregateId);
